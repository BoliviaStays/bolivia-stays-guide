(() => {
  ...
  function render() {
    ...
    document.querySelector('main').innerHTML = `
      ...
      <section class="section" id="checkout"><div class="section-heading"><p class="eyebrow">${esc(ui.checkout)}</p><h2>${esc(ui.beforeLeaving)} <span class="time">11:00</span></h2></div><ul class="checkout-list">${ui.checkoutItems.map(text => `<li>${esc(text)}</li>`).join('')}${property === '11f' ? `<li>${esc(p.notice)}</li>` : ''}</ul>${detail(base.faq.items[1][0],base.faq.items[1][1])}${detail(base.faq.items[2][0],base.faq.items[2][1])}</section>
